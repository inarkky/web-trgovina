...
$kategorija=mysqli_fetch_array(mysqli_query($db_conx, "SELECT * FROM proizvod WHERE akcija=1"));
if(count($kategorija)<1){
  $render = '<li>Na zalost trenutno nema proizvoda na akciji!</li>';
}else{
$rez_p_page=9;
if (isset($_GET['page'])) {
...
$render='';
$sql="SELECT * FROM proizvod WHERE akcija=1 AND Kol_lager>0 LIMIT $start, $rez_p_page";
...
<div class="middle">
  ...
  <?php include_once('sidebar.php'); ?>
   <div class="col-md-9">
   <h2 align="center">Najnovije na akciji</h2><br>
...
$page_sql = "SELECT * FROM proizvod WHERE akcija=1 AND Kol_lager>0";
$result = mysqli_query($db_conx, $page_sql);
$rows_total = mysqli_num_rows($result);
$pages_total = ceil($rows_total / $rez_p_page);

echo "<center><a class='btn btn-default' href='akcije.php?page=1'>Prva</a> ";

for ($i=1; $i<=$pages_total; $i++) {

echo "<a class='btn btn-default' href='akcije.php?page=" . $i . "'>" . $i . "</a> ";
};

echo "<a class='btn btn-default' href='akcije.php?page=" . $pages_total . "'>Zadnja</a></center> ";
 }
?>
